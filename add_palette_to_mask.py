...
_palette = [  # (256 * 3)
    ...
    252, 252, 252, 253, 253, 253, 254, 254, 254, 255, 255, 255, 255, 0, 0
]
...
def main():
    ...
    parser.add_argument("--mask_folder", type=str, default="annotations/masks")
    parser.add_argument("--output_folder", type=str, default="annotations/colored_masks")
    args = parser.parse_args()

    args.videos = [f"{i:04d}" for i in range(2, 31)]
    for video in args.videos:
        video_path = os.path.join(args.dataset, video)
        mask_folder = os.path.join(video_path, args.mask_folder)
        output_folder = os.path.join(video_path, args.output_folder)
        ...
        for mask_file in tqdm(mask_files):
            ...
            assert np.unique(mask)[-1] < 256, "The largest object id cannot be greater than 255."
            mask = mask.astype(np.uint8)

            pred_label = Image.fromarray(mask.astype(np.uint8)).convert("P")
            pred_label.putpalette(_palette)
            pred_label.save(os.path.join(output_folder, mask_file))
